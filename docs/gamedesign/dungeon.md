...
Пол и стены рендерятся через `Polygon2D` с текстурой + `texture_repeat = ENABLED`:
...
UV каждой поверхности берётся от **абсолютных координат этажа**, не от размера самого polygon'а. За счёт этого стыки между комнатами и коридорами, а также между смежными wall-span'ами по строке — бесшовные, узор кирпичей продолжается непрерывно.

Тайлы генерируются детерминированно из `tools/gen_environment_sprites.py` (Pillow, палитра + матрица). Правки — как обычно: обновляй палитру/матрицу в скрипте, запускай `python3 tools/gen_environment_sprites.py`, коммить и PNG, и скрипт.
