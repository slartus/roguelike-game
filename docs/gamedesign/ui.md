...
Клавиша `ESC` (input action `pause`) в `hud.gd::_unhandled_input` вызывает `_toggle_pause`: переключает `get_tree().paused` и `visible` панели `PausePanel` (полноэкранный `ColorRect` с alpha 0.6 и центрированным `VBoxContainer` `PauseBox` с заголовком «ПАУЗА» + четырьмя строками статистики текущего забега).

**Статистика текущего забега на паузе** (`_refresh_pause_stats`): при включении паузы labels обновляются через `tr("UI_RUN_STATS_*")`, показывая `GameState.current_floor_number`, `player_level`, `run_enemies_killed`, `run_gold`. Ключи tr() shared с окном «Итоги забега» на title screen — одна семантика. Разница: пауза показывает **текущий прогресс** (`run_*` + `current_*`), title screen — **snapshot прошлого забега** (`last_run_*`).
