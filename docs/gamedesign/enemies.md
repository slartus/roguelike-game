...
- **LANDED** — приземлилась на `target_position`; коллизия раздувается до r=14, живёт `LANDED_LIFETIME` секунд и `queue_free`. Пока игрок стоит внутри — его скорость умножается на `SLOW_FACTOR` = 0.5. Финальный fade-out последние 25% жизни. Визуал: полупрозрачный диск-подложка + `WEB_SPOKE_COUNT = 8` радиальных нитей от центра к внешнему кольцу + `WEB_RING_COUNT = 3` концентрических кольца, нарисованных `draw_polyline` — получается настоящая паутина, а не белая клякса.
