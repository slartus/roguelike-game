...
| `weapon_equipped`          | `Analytics.record_weapon_equipped(id, prev, source)` — старт/пикап/сундук |
| `upgrade_offer_shown`      | Панель выбора карты открыта (`upgrade_choice_panel._open_panel`) |
| `upgrade_selected`         | Игрок выбрал карту                                       |
| `potion_used`              | Активирован potion слот 1                                |
| `room_first_entered`       | Первый визит в комнату на этаже (см. `Analytics.record_room_entered`) |
| `floor_weapon_summary`     | По одному на каждое использованное оружие в конце этажа  |
| `floor_enemy_summary`      | По одному на каждую (enemy_id, temperament, elite_rank) группу |
| `floor_economy_summary`    | Один агрегированный event с gold-source split и potions  |
...
{
  "weapon_id": "spear",
  "previous_weapon_id": "dagger",
  "source": "pickup"
}
```

`source` ∈ `{"starting", "pickup", "chest", "debug", "other"}`.
...
Событие эмитится один раз на комнату за этаж (при первом визите).

### `floor_weapon_summary`

По одной записи на weapon_id, использованное на этаже:

```json
{
  "weapon_id": "dagger",
  "equipped_seconds": 80.4,
  "combat_seconds": 42.2,
  "attacks": 110,
  "projectiles_fired": 0,
  "attacks_with_hit": 70,
  ...
  "kills": 18,
  "overkill_damage": 0,
  "damage_taken_while_equipped": 3
}
```

`combat_seconds` в PR 2 = 0 (упрощение). `overkill_damage` в PR 2 всегда 0 —
для точного значения нужен доступ к remaining_hp в момент летального удара,
это оставлено как follow-up.

### `floor_enemy_summary`

По одной записи на группу `(enemy_id, temperament_id, elite_rank)`:

```json
{
  "enemy_id": "goblin",
  "temperament_id": "cautious",
  "elite_rank": 0,
  "spawned": 4,
  "killed": 4,
  "damage_to_player": 3,
...
`time_alive_seconds` в PR 2 = 0 (нет полного tracking'а per-instance lifetime).

### `floor_economy_summary`

Один event на этаж, агрегирует gold-sources и потребление зелий:
...
## Damage attribution (PR 2)

`DamageContext` — RefCounted-класс со всей информацией об одном damage-событии.
Заполняется на стороне attacker'а (weapon/enemy/projectile/ability) и
передаётся как второй параметр в `take_damage(amount, context)`.

Analytics использует контекст для:
- **weapon damage_dealt / kills** — сколько урона нанесло конкретное оружие;
- **enemy damage_to_player / damage_received** — сколько урона враг наносит и получает;
- **death attribution** — «умер от `spider` `charge` `aggressive` `elite_rank=1`»;
- **damage_history** — последние `DAMAGE_HISTORY_LIMIT=16` событий против игрока.

### Правила hit counting

| Действие                | Что инкрементируется                    |
|-------------------------|-----------------------------------------|
| Melee swing начат       | `attacks += 1`                          |
| Melee swing попал ≥ в 1 | `attacks_with_hit += 1` (max 1/swing)   |
| Melee попал в 3 цели    | `targets_hit += 3`                      |
| Ranged: 1 pellet выпущен| `projectiles_fired += 1`                |
| Ranged: 3 spread pellets| `projectiles_fired += 3`                |
| Projectile попал в цель | `projectiles_hit += 1` (один раз/projectile) |
| Pierce projectile 2 hits| `targets_hit += 2`, `projectiles_hit += 1` |
| DOT tick (poison_tick)  | `damage_taken` + `damage_to_player`     |

### Factory helpers `DamageContext`

- `from_player_weapon(weapon, target)` — melee swing / projectile hit → enemy.
- `from_enemy_attack(enemy, attack_id)` — contact / reach / charge → player.
- `from_enemy_projectile(source_enemy, attack_id)` — projectile → player.
- `from_enemy_ability(source, attack_id)` — poison_tick / aoe → player.
- `unknown()` — fallback для legacy path (attribution="unknown").

## Weapon / Upgrade / Enemy analytics (PR 2)

Классы `WeaponAnalyticsCounters`, `EnemyAnalyticsCounters`, `EconomyCounters`
живут в `analytics/`. Хранятся в `RunAnalyticsState`:

- `floor_weapon_counters: Dictionary[StringName → WeaponAnalyticsCounters]` — сброс на `start_floor`;
- `run_weapon_counters: Dictionary` — не сбрасывается, идёт в `run_finished.weapon_totals`;
- `floor_enemy_counters: Dictionary[key → EnemyAnalyticsCounters]` — сброс на `start_floor`;
- `economy: EconomyCounters` — сброс на `start_floor`.

**Enemy grouping key:** `"enemy_id|temperament_id|elite_rank"`. Разные темпераменты
одного enemy — разные строки в `floor_enemy_summary`.

## `run_finished` — расширенный payload (PR 2)
...
```json
{
  ...
  "death_source_type": "enemy",
  "death_source_id": "spider",
  "death_attack_id": "charge",
  "death_source_temperament": "aggressive",
  "death_source_elite_rank": 1,

  "equipped_weapon_id": "spear",
  "potions_remaining": 1,
  "upgrade_stacks": {"heavy_strike": 2, "long_reach": 1},

  "damage_history": [
    {"source_id": "zombie", "attack_id": "poison_tick", "damage": 1, "ticks_ms": 82000, "floor": 3},
    ...
  ],
  "weapon_totals": [
    {"weapon_id": "spear", "equipped_seconds": 340, "attacks": 512, "kills": 68, ...},
    {"weapon_id": "dagger", "equipped_seconds": 45, "attacks": 60, "kills": 4, ...}
  ]
}
```

Поля `death_source_*` присутствуют только если `reason=player_death` и
`last_damage_context != null`.

## Ограничения PR 2

Целенаправленно НЕ включено (follow-up для PR 3 или дальше):

- полные ability-specific counters (slime jump attempts, spider charges, lich summons и т.п.) — есть `attack_id` в contexts, но per-enemy raw counter'ов пока нет;
- floor layout graph metrics (`critical_path_length_cells`, `dead_end_count`, `loop_count`) — есть только `room_count`/`enemy_count`/`chest_count`;
