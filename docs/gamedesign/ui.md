...
4. Раскладываем весь `WEAPON_ROSTER` (все 9 `.tres` из `resources/weapons/`) в ряд вдоль верхней стены на `y = 48`: `x = wall_thickness + slot_width * (i + 0.5)`, где `slot_width = inner_width / roster_size`. Порядок соответствует порядку в коде — читается слева направо. Каждый пикап подписывает `tree_exited` на `_on_pickup_taken(slot_index)`, чтобы после подбора в том же слоте появился новый экземпляр того же оружия — сандбокс никогда не пустеет.

HUD:

- `HintLabel` (`UI_DEBUG_WEAPON_HINT`) — подсказка управления вверху.
- `WeaponInfoPanel` — VBoxContainer в правом-нижнем углу, `EmptyLabel` (`UI_DEBUG_WEAPON_INFO_EMPTY`) виден пока `equipped_weapon == null`; при экипировке скрывается, `StatsBox` показывает 5 строк: название (`tr(weapon.display_name)`), урон (`UI_DEBUG_WEAPON_DAMAGE`), дальность (`UI_DEBUG_WEAPON_RANGE`, `weapon.attack_range`), угол (`UI_DEBUG_WEAPON_ANGLE`; для `melee_arc/melee_thrust` — `arc_degrees`, для `projectile/spell_projectile` — `spread_angle_deg`), скорость (`UI_DEBUG_WEAPON_SPEED`, `1 / get_attack_interval()` — ударов в секунду). Панель подписана на `player.weapon_changed` и обновляется при каждой смене оружия.
- `PlayerPreview` — Node2D в мировом слое (не под CanvasLayer), `scale = Vector2(3, 3)`, позиция в левом-нижнем углу комнаты. Содержит `PreviewVisual` и `PreviewWeapon` — Sprite2D, которые в `_process` зеркалят `texture`, `flip_h`, `modulate`, `position`, `rotation` и `visible` соответствующих нод реального Player (`Visual` и `Weapon`). Смысл — увидеть анимацию удара, поворот оружия и rest-tilt в 3× масштабе, не приближаясь к самому персонажу камерой.

ESC (`ui_cancel`) возвращает на `debug_menu.tscn`.

Тест `test/unit/test_weapon_test_screen.gd::test_all_weapons_in_resources_dir_are_included_in_roster` защищает от рассинхрона: если в `resources/weapons/` добавили новый `.tres`, но `WEAPON_ROSTER` не обновили — тест красный. `test_pickup_respawns_in_same_slot_after_being_taken` фиксирует respawn-контракт, `test_weapon_info_panel_*` — контракт панели статов.
