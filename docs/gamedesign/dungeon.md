...
`floor_number % 5 == 0` → одна большая арена. Размер приходит из `BossArenaProfile.size` через `BossRegistry.arena_profile_for_floor(floor)`; при `Vector2i.ZERO` (профиль без размера) генератор откатывается на `BOSS_ROOM_SIZE = 600×400`.

Текущие профили:

- **Этаж 5 — Castellan Hall (640×420 px, `residential`)** — прямоугольный парадный зал Кастеляна. Половина меньшей оси (210 px) меньше `CHARGE_SPEED × CHARGE_MAX_DURATION = 352 px`, поэтому charge босса из центра гарантированно уткнётся в стену → стабильное `wall stun` окно для игрока. `clear_center_radius = 96 px`. См. `docs/gamedesign/enemies.md → Castellan Armor`.
- **Этаж 10 — Rune Engine Chamber (620×420 px, `technical`)** — центральный овал под Rune Golem'а; 6 lane layouts с шагом 105 px гарантированно вписываются в footprint. `clear_center_radius = 84 px`. См. `docs/gamedesign/enemies.md → Rune Golem`.
- **Этаж 15 — Ritual Crypt (680×460 px, `basement`)** — самая большая арена: босс + 5 миньонов + игрок должны сосуществовать с aimed/radial снарядами и не оказываться в бутылочном горлышке. `clear_center_radius = 110 px`. См. `docs/gamedesign/enemies.md → Necromancer`.
- **Этаж 20 — Ritual Crypt (fallback)** — до PR 5 (Crystal Wyrm) 20-й этаж обслуживается тем же Necromancer'ом как fallback definition; используется тот же ritual_crypt-профиль арены.

Boss-этаж пропускает всю ветвь room_graph / encounter budget / rewards — boss спавнится напрямую в `main.gd::_spawn_boss` через definition из `BossRegistry`.
