...
1. `WAITING` — стоит `wait_duration` секунд (светлее оттенок через `modulate`). **В момент входа в WAITING** паук плюёт паутиной (`spider_web.tscn`) в текущую позицию игрока — цель фиксируется на этом моменте, не хоминг.
...
| `LANDED_LIFETIME` | 6.0 s |
| `SLOW_FACTOR` (player) | 0.5 |

- **FLYING** — летит к `target_position` со скоростью `FLIGHT_SPEED`. Коллизия маленькая (r=3); пролёт над игроком **не** триггерит slow — иначе сам факт стрельбы был бы двойным ударом.
- **LANDED** — приземлилась на `target_position`; коллизия раздувается до r=14, живёт `LANDED_LIFETIME` секунд и `queue_free`. Пока игрок стоит внутри — его скорость умножается на `SLOW_FACTOR` = 0.5. Финальный fade-out последние 25% жизни.
