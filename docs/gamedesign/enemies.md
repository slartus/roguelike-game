...
Не дропает пикапы — награда идёт через XP/gold. Появляется каждые 5 комнат.

## Пул спавна

`Main.ENEMY_SCENES` содержит все 8 обычных типов (без босса). Спавн случайный, равновероятный (`pick_random()`). Босс появляется в отдельной ветке `Main._is_boss_room()`.

Количество врагов на комнату:
```
target_count = clamp(MIN_ENEMIES + room_number - 1, MIN_ENEMIES, MAX_ENEMIES)
```
где `MIN_ENEMIES = 3`, `MAX_ENEMIES = 6`.
