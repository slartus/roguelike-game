...
5. **Только после зелёных тестов и READY-TO-COMMIT вердикта** делать `git commit`.

Освобождение: правки, которые не трогают `.gd` / `.tscn` / `.tres` (например, только `docs/*.md` или `CLAUDE.md`) освобождаются от шагов 2 и 3. Шаги 1 и 4 остаются в силе, если правка затрагивает game design.
