...
**Призыв скелета (`lich.gd::_update_summon`).** Помимо стрельбы magic-bolt лич поддерживает одного скелета-миньона.

| Параметр | Значение |
|----------|----------|
| SUMMON_COOLDOWN | 5.0 s |
| SUMMON_OFFSET_MIN | 24 px |
...
- Первый призыв — через 5 с после спавна лича (`_summon_cooldown_timer` стартует полным, не нулевым). Даёт игроку окно среагировать.
- Когда миньон убит и ссылка невалидна — таймер снова тикает от 0 до 5 s, потом новый призыв.
- Позиция: `global_position + rand_direction * randf_range(24, 40)` — вокруг лича в случайном направлении, проверенная валидатором `_pick_valid_spawn_position`: до `SPAWN_ATTEMPTS = 12` кандидатов, отбрасываются те, чья клетка `AStarGrid2D.is_point_solid` (стена) или `not is_in_boundsv` (за пределами этажа). Если все 12 попыток провалились — тик пропускается, `_summon_cooldown_timer` остаётся ≤ 0, следующий physics-тик попробует снова. Без валидации баг: скелет иногда спавнился внутри стены и застревал в геометрии, оставаясь неубиваемым.
