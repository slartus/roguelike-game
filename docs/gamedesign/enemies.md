...
**Почкование при агре.** При первом переходе `WANDER→CHASE` (агра — увидел игрока или получил урон) слайм запускает таймер `BUD_DELAY` = 2.0 s. По истечении рядом со слаймом появляется ещё один слайм («почка»). Каждый слайм почкуется максимум один раз (`_has_budded`), даже если после первой попытки он потерял цель, ушёл в WANDER и агрился повторно. Дочерний слайм — обычный, с собственным `_has_budded = false`, поэтому и сам сможет отпочковаться при первом собственном агре.
